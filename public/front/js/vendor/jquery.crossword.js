			...
			*/
			...
			var puzInit = {
				...
				*/
				buildEntries: function() {
					...
					util.highlightEntry();
					util.highlightClue();
					$('.crossword .active').eq(0).focus();
					$('.crossword .active').eq(0).select();
			...
			var nav = {
				...
				updateByNav: function(e) {
					...
					$('.crossword .active').removeClass('active');
					$('.current').removeClass('current');
					currIndex = 0;
					...
					$('.crossword .active').eq(0).focus();
					$('.crossword .active').eq(0).select();
					$('.crossword .active').eq(0).addClass('current');
